...
import static net.minecraft.util.math.BlockPos.getAllInBoxMutable;
	...
	public static void freezeNearby(EntityPlayer living, World worldIn, BlockPos pos) {
		...
		try {
			if (living != null) {
				...
				for (MutableBlockPos blockPos : getAllInBoxMutable(pos.add((double) (-f), -1.0D, (double) (-f)),
					...
					if (blockPos.distanceSqToCenter(living.posX, living.posY, living.posZ) <= (double) (f * f)) {
						...
						if (iblockstate.getMaterial() == Material.AIR) {
							...
							if (iblockstate1.getMaterial() == Material.WATER
									&& (iblockstate1.getBlock() == Blocks.WATER
											|| iblockstate1.getBlock() == Blocks.FLOWING_WATER)
									&& ((Integer) iblockstate1.getValue(BlockLiquid.LEVEL)).intValue() == 0
									&& worldIn.mayPlace(Blocks.ICE, blockPos, false, EnumFacing.DOWN, (Entity) null)) {
								worldIn.setBlockState(blockPos, Blocks.ICE.getDefaultState());
								worldIn.scheduleUpdate(blockPos.toImmutable(), Blocks.ICE,
										MathHelper.getInt(living.getRNG(), 60, 120));
							} else if (iblockstate1.getMaterial() == Material.LAVA
									&& (iblockstate1.getBlock() == Blocks.LAVA
											|| iblockstate1.getBlock() == Blocks.FLOWING_LAVA)
									&& ((Integer) iblockstate1.getValue(BlockLiquid.LEVEL)).intValue() == 0 && worldIn
											.mayPlace(Blocks.MAGMA, blockPos, false, EnumFacing.DOWN, (Entity) null)) {
								worldIn.setBlockState(blockPos, Blocks.MAGMA.getDefaultState());
								worldIn.scheduleUpdate(blockPos.toImmutable(), Blocks.MAGMA,
										MathHelper.getInt(living.getRNG(), 60, 120));
							}
							...
							if (iblockstate1.getBlock() == Blocks.SNOW) {
								...
								worldIn.scheduleUpdate(blockPos.toImmutable(), Blocks.PACKED_ICE,
										MathHelper.getInt(living.getRNG(), 60, 120));
							} else if (iblockstate1.getBlock() == Blocks.STONE) {
								worldIn.setBlockState(pooledMutableBlockPos,
										BlocksRegistry.WinterStone.getDefaultState());
								worldIn.scheduleUpdate(blockPos.toImmutable(), BlocksRegistry.WinterStone,
										MathHelper.getInt(living.getRNG(), 60, 120));
							}
						}
					}
