...
public class LegendaryWinter {
  ...
  static {
    ...
    POLARIUM_HORSE_ARMOR =
      EnumHelper.addHorseArmor(
        new ResourceLocation(MODID, "polarium").toString(),
        // DIAMOND(11, "diamond", "dio");
        new ResourceLocation(
          MODID,
          "textures/entities/horse/armor/horse_armor_polarium.png"
        )
          .toString(),
        12
      );
  ...
  public void init(FMLInitializationEvent event) {
    proxy.init(event);
    // in here to make sure polarium_ingot exists (registry events are fired right
    // before init)
    POLARIUM_TOOL_MATERIAL.setRepairItem(
      new ItemStack(ItemsRegistery.polarium_ingot)
    );
    POLARIUM_ARMOR_MATERIAL.setRepairItem(
      new ItemStack(ItemsRegistery.polarium_ingot)
    );
